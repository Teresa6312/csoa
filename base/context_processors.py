...
def default_context(request):
    ...
    if not(request.path.startswith('/accounts/') or request.path.startswith('/admin/')):
        current_page_menu = request.current_page_menu if hasattr(request, 'current_page_menu') else None
        permission_list = request.permission_list if hasattr(request, 'permission_list') else None
        context['app_tree'] = request.app_tree if hasattr(request, 'app_tree') else None
        context['current_page_menu'] = current_page_menu 
        context['level_1_menu'] = request.level_1_menu if hasattr(request, 'level_1_menu') else None
        context['permission_list'] = permission_list
        permission_context = get_user_current_page_menu_permission_context(request, current_page_menu, permission_list)
        context.update(permission_context)
    return context

def get_user_current_page_menu_permission_context( request, current_page_menu, permission_list):
