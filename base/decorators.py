...
def request_decorator(func):
    ...
    def wrapper(request, *args, **kwargs):
        ...
        try:
            ...
            return JsonResponse({'message_type': 'error', 'message': field_errors}, status=400)
        ...
        if message is not None:
            ...
            if message is not None:
                if request.path.startswith('/api'):
                    return JsonResponse({"message_type": "error", "message": message, "data": []}, status=404)
...
def __setup_model(request, context, model, id):
    ...
    if model is not None:
        ...
        if model_details is not None:
            model_class =  get_model_class(model_details.get('backend_app_label', ''),  model_details.get('backend_app_model', ''))
