...
class MenuMiddleware:
    ...
    def __call__(self, request):
        ...
        if request.user.is_authenticated:
            user_info = request.session['user_info'] if 'user_info' in request.session else None
        else:
            user_info = None
        ...
        if request.path.startswith('/accounts/') or \
           (request.path.startswith('/admin') and user_info is not None and user_info.get('is_superuser', False) and user_info.get('is_staff', False)) or \
           request.path == '/favicon.ico' or \
           request.path.startswith('/static/') or \
           request.path.startswith('/global/') or \
            request.path.startswith('/api/global/'):
        ...
        if data is not None:  # Retrieve from cache if available
            ...
            if request.path not in ('/', ''):
                current_page_menu = self.get_current_page_menu_for_user(menu_tree, new_path)
                ...
                current_app_key = role_unit[0].get('permission_role__app__key') if len(role_unit) > 0 else None
                level_1_menu = self.get_level_1_menu(request, app_tree, current_app_key, user_info)
    ...
    def get_app_tree_for_user(self, request, user_info):
        ...
        if user_info.get('is_superuser', False):  # Check if user is superuser using get method to avoid KeyError
    ...
    def get_permission_list(self, current_page_menu):
        ...
        return get_menu_key_in_list(sub_menu) if sub_menu else [] # More concise way to check and return
