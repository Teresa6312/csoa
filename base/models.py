...
class DictionaryModel(BaseAuditModel):
    ...
    @global_class_cache_decorator(cache_key='dictionary', timeout=settings.CACHE_TIMEOUT_L3)
    ...
    @global_class_cache_decorator(cache_key='dictionary_active', timeout=settings.CACHE_TIMEOUT_L3)
...
class DictionaryItemModel(BaseAuditModel):
    ...
    @global_class_cache_decorator(cache_key='dictionary_item_map_active_by_code', timeout=settings.CACHE_TIMEOUT_L3)
    def get_active_dictionary_item_map_by_code(cls, code):
        queryset = cls.objects.filter(
            dictionary=code, is_active=True, 
            dictionary__is_active=True, 
        )
        dic_query_name = 'sub_category' 
        dic_item_query_name = 'sub_category__dictionary_item_dictionary' 
        sub_set = queryset.filter(**{'%s__is_active'% dic_query_name: True, '%s__is_active'% dic_item_query_name: True})
        annotated_fields = {}
        fields = ['code', 'value']
        index = 1
        while sub_set.exists():
            ...
            annotated_fields['L%s__value'% index] = F('%s__value'% dic_item_query_name)
            fields.append('L%s__code'% index)
            fields.append('L%s__value'% index)
            dic_query_name = 'sub_category__dictionary_item_dictionary__' + dic_query_name
            dic_item_query_name = 'sub_category__dictionary_item_dictionary__sub_category__dictionary_item_dictionary'+ dic_item_query_name
            sub_set = sub_set.filter(**{'%s__is_active'% dic_query_name: True, '%s__is_active'% dic_item_query_name: True})
            index += 1
    ...
    def get_dictionary_item_map_by_code(cls, code):
        ...
        dic_item_query_name = 'sub_category__dictionary_item_dictionary'
        queryset = queryset.prefetch_related(dic_item_query_name + '__' + dic_item_query_name) # Optimize query
        ...
        while sub_set .exists() and index < 5:
            ...
            queryset = queryset.prefetch_related(dic_item_query_name + '__sub_category') # Optimize query
            ...
            dic_item_query_name = 'sub_category__dictionary_item_dictionary__'+ dic_item_query_name
...
class ModelDictionaryConfigModel(BaseAuditModel):
    ...
    json_form = models.ForeignKey('jsonForm.FormTemplate', on_delete=models.CASCADE, blank=True, null= True)
