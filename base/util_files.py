    ...
    if isinstance(file, list):
        ...
        for f in file:
            ...
            file_instance = FileModel.objects.create(
                name=safe_filename, file=f, created_by=request.user.username
            )
            file_list.append(file_instance)
    else:
        safe_filename = get_valid_filename(file.name)
...
def handle_single_file(request, file, file_path, format="json"):
    ...
    if isinstance(file, dict):
        ...
        try:
            with open(file["file_path"], "rb") as file_obj:
                ...
                new_file = ContentFile(
                    new_file_content, name="%s/%s" % (file_path, file["name"])
                )
        ...
        safe_filename = get_valid_filename(file.name)
        file_instance = FileModel.objects.create(
            name=safe_filename, file=new_file, created_by=request.user.username
        )
    else:
...
def process_formset_files(request, formset):
    for field_name, field in formset.fields.items():
        if isinstance(field, forms.FileField) or isinstance(field, MultipleFileField):
            input_name = (
                f"{formset.prefix}-{field_name}"
                if formset.prefix is not None
                else field_name
            )
...
def process_form_files(request, form):
    previous_files = (
        request.session.get("uploaded_files")
        if hasattr(request.session, "uploaded_files")
        else None
    )
    for field_name, field in form.fields.items():
        if isinstance(field, forms.FileField) or isinstance(field, MultipleFileField):
            input_name = (
                f"{form.prefix}-{field_name}" if form.prefix is not None else field_name
            )
            clear_files = request.POST.get(f"{input_name}-clear")
            old_files = (
                field.initial
                if previous_files is None
                else previous_files.get(field_name)
            )
            uploaded_files = request.FILES.getlist(input_name)
            # handle  any file uploaded
            if uploaded_files is not None and len(uploaded_files) > 0:
