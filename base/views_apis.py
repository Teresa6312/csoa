...
def get_case_details_file_download_view(request, context, app_name, form_code, case_id ,file_id):
	...
	if data is None or data.count()==0:
		data = case_instance.workflow_instance.task_instance_workflow_instance.filter(files__id=file_id)
	...
	return Response({"message": "The requested file was not found on the server", "message_type": "error"})
...
def get_case_details_documents_json(request, context, app_name, form_code, case_id):
	...
	if case_instance.workflow_instance is not None:
		task_ids = case_instance.workflow_instance.task_instance_workflow_instance.all().values('id')
		...
		data = FileModel.objects.filter(Q(**{f'{related_name}__in': task_ids})).order_by('-created_at').values(*fields)
