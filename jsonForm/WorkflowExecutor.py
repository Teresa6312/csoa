...
class WorkflowExecutor:
    def execute(self, case, current_task):
        ...
        while current_task:  # Loop for chained auto tasks
            if current_task.task_type == constants.TASK_TYPE_AUTO:
                ...
                next_task = self.execute_auto_task(current_task, request_data, task_instance)  # Execute the auto task
                ...
                self.create_flow_task_instance(application, case, current_task, workflow_instance)  # Handle flow task
                return current_task
            elif current_task.task_type == constants.TASK_TYPE_MANUAL:
                raise ValueError(f"Next task should not be Manual Task, Manual Task is for user to create")  # Manual tasks are user-created
            else:
                raise ValueError(f"Unknown task type: {current_task.task_type}")  # Handle unknown task types
    ...
    def create_flow_task_instance(self, application, case, task, workflow_instance):
        ...
        if task.assign_to_role is not None:
            per = Permission.get_assign_to_role(application, task.assign_to_role, case)  # Get Permission by role
            if per is not None:
                pers.append(per)
        elif task.assign_to is not None:
            pers = task.assign_to.all()  # Get Permissions directly
        if len(pers) == 0:
            raise ValueError(f"No user group was found to assign the task ({task.assign_to_role} or {task.assign_to}). If the task is assign to the case/request owner, it may has lost the access")  # Handle no assignees
    ...
    def get_priority_decision(self, case):
        ...
        try:
            ...
            return decision_points[0] if len(decision_points)>0 else None  # Return the highest priority or None
    ...
    def execute_auto_task(self, task, request_data, task_instance):
        ...
        """
        decision_points = task.decision_points_task.order_by('priority')  # Order by priority
        for dp in decision_points:
            if self.evaluate_condition(dp.condition, request_data, task_instance):  # Evaluate the condition
                ...
                try:  # Extract matched values for the comment
                    ...
                    if isinstance(dp.condition, str): # Check if it's a string (older Django versions might store JSON as strings)
                        condition_data = json.loads(dp.condition)  # Load JSON
                    if isinstance(dp.condition, dict): # Make sure it's a dictionary before merging
                    ...
                    logger.error(f"Invalid JSON format in condition: {dp.condition}. Error: {e}")  # Log the error
    ...
    def evaluate_condition(self, condition_json, request_data, task_instance):
        ...
        try:
            # Safely load JSON data.  Handles cases where the JSONField might be None or contain invalid JSON
            if isinstance(condition_json, str): # Check if it's a string (older Django versions might store JSON as strings)
                condition_data = json.loads(condition_json)  # Load JSON
            if isinstance(condition_json, dict): # Make sure it's a dictionary before merging
                condition_data = condition_json
        except json.JSONDecodeError as e:
            task_instance.comment = f"[ERROR] Invalid Task Condition format json: {e}"  # Error comment
            task_instance.save()
            logger.debug(f"Invalid Task Condition format json: {condition_json}. Error: {e}")  # Log
            raise ValueError(f"Invalid Task Condition format json: {condition_json}. Error: {e}")  # Raise error
            # return False
        ...
        def build_query(cond):
            ...
            compare_value = cond.get("compare_value")  # 使用get方法，避免key不存在的错误
            actual_value = request_data.get(field_name)

            if actual_value is None and comparison_operator != "isnull":
                return Q()  # 如果actual_value不存在，且不是isnull查询，则直接返回空Q对象
            ...
            if comparison_operator == "in":
                if isinstance(compare_value, list):
                    ...
                    task_instance.save()
                    logger.debug(f"Invalid compare_value for 'in' operator: {compare_value} in condition: {cond}") # 添加日志
                    return Q()
                ...
                kwargs = {f"{field_name}__isnull": compare_value} #  compare_value 应该为布尔值
        ...
        if "operator" in condition_data and "conditions" in condition_data:
            ...
            for condition in conditions:
                ...
                if operator == "AND":
                    ...
                    return False
        elif "field_name" in condition_data and "comparison_operator" in condition_data:  # 兼容单个condition的情况
            q = build_query(condition_data)
        else:
            task_instance.comment = f"[ERROR] Invalid Task Condition format {condition_json}"
            task_instance.save()
            logger.error(f"Invalid operator: {operator} in condition: {condition_json}")
            return False
