...
class CaseForm(forms.Form):
    ...
    case_team = forms.ModelChoiceField(
        queryset=Team.objects.filter(is_active=True),
        required=False,
        label="Initial By Team",
    )
    case_department = forms.ModelChoiceField(
        queryset=Department.objects.filter(is_active=True),
        required=False,
        label="Initial By Department",
    )
    ...
    def create_case_form(app_id, user, *args, **kwargs):
        ...
        if app_id is not None and not hasattr(case_form, "cleaned_data"):
            ...
            if app.control_type == "team":
                ...
                case_form.fields["case_team"].queryset = Team.objects.filter(
                    Q(permission_team__app=app_id) | Q(user_team__id=user.id)
                ).distinct()
                ...
                case_form.fields["case_department"].queryset = (
                    Department.objects.filter(
                        Q(permission_department__app=app_id)
                        | Q(user_department__id=user.id)
                    ).distinct()
                )
                case_form.fields["case_department"].initial = user.department
                case_form.fields["case_department"].required = True
            case_form.fields["created_by"].queryset = CustomUser.objects.filter(
                Q(permissions__app=app_id) | Q(id=user.id)
            ).distinct()
...
def create_dynamic_form_section(json_str, form_section_id=None, instance=None):
    ...
    class DynamicFormSection(forms.Form):
        ...
        def __init__(self, data=None, files=None, *args, **kwargs):
            ...
            initial = (
                self.initial or kwargs.get("initial", {})
                if instance is None
                else instance
            )
            if form_section_id is not None:
                self.fields["form_section_id"] = forms.IntegerField(
                    initial=form_section_id, widget=forms.HiddenInput()
                )
            ...
            for field_name, field_props in field_definitions.items():
                ...
                field_default = (
                    field_props.get("default", None)
                    if initial.get(field_name, None) is None
                    else initial.get(field_name)
                )
                ...
                if choices_map != {}:
                    new_field_choices = get_select_choices_from_map(
                        choices_map.get("map_name"), choices_map.get("map_key")
                    )
                ...
                if field_type == "string":
                    ...
                    self.fields[field_name] = forms.DecimalField(
                        ...
                        validators=validators,
                        widget=forms.NumberInput(
                            attrs={"placeholder": field_placeholder, "step": "0.01"}
                        ),
                    )
                    ...
                    self.fields[field_name] = forms.DateField(
                        ...
                        widget=forms.DateInput(
                            attrs={"placeholder": field_placeholder, "type": "date"}
                        ),  #
                    )
                    ...
                    if field_props.get("multiple", False):
                        ...
                        self.fields[field_name] = forms.FileField(
                            ...
                            widget=CustomClearableFileInput(
                                original_required=field_required
                            ),
                        )
                if field_hidden:
...
def create_dynamic_form_section_formset(
    fields,
    ...
    if post_data is not None:
        formset = DynamicFormSectionFormSet(
            post_data, post_file, prefix=prefix, initial=instance
        )
    else:
...
class FormTemplateForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance and self.instance.pk and self.instance.application is not None:
            queryset = AppMenu.objects.filter(
                Q(menu_level=0) | Q(pk=self.instance.application.pk)
            )
        else:
            queryset = AppMenu.objects.filter(menu_level=0)
        ...
        if self.instance and self.instance.pk and self.instance.workflow is not None:
            queryset = Workflow.objects.filter(
                Q(is_active=True) | Q(pk=self.instance.workflow.pk)
            )
        else:
    ...
    def save(self, commit=True):
        ...
        if (
            instance.backend_app_label is None
            or instance.backend_app_model is None
            or instance.backend_app_section_model is None
        ):
        ...
        if new_link_app:
            ...
            search.save()
            my_cases_menu = AppMenu.objects.filter(
                parent_app_menu=instance.application, key="my_cases"
            ).first()
            ...
            for i, section in enumerate(sections):
                if section.get("is_active", False) and section.get("is_publish", False):
                    ...
                    for other_section in sections[i + 1 :]:
                        if other_section.get("is_active", False) and section.get(
                            "is_publish", False
                        ):
                            new_keys = set(
                                other_section.get("json_template", {}).keys()
                            )
...
class TaskForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields["assign_to"].queryset = Permission.objects.filter(
            role__isnull=False
        ).order_by("app")
    ...
    def clean(self):
        ...
        if (
            ...
            and not self.cleaned_data.get("DELETE", False)
        ):
            data = self.cleaned_data
            if data.get("task_type", "") == constants.TASK_TYPE_FLOW:
                if (
                    len(data.get("assign_to", [])) == 0
                    and data.get("assign_to_role", None) is None
                ):
                    raise ValidationError(
                ...
                if (
                    len(data.get("assign_to", [])) != 0
                    and data.get("assign_to_role", None) is not None
                ):
                    raise ValidationError(
    ...
    class TaskInstanceForm(forms.ModelForm):
        ...
        def __init__(self, *args, **kwargs):
            ...
            if self.instance.id:
                ...
                self.fields["decision_point"].queryset = DecisionPoint.objects.filter(
                    task=task.task
                )

                if self.request.user.is_superuser:
                    self.fields["assign_to_user"].required = True
                    self.fields["assign_to_user"].queryset = CustomUser.objects.filter(
                        permissions=assign_to, is_active=True
                    )
                else:
                    self.fields["assign_to_user"].widget = forms.HiddenInput()
                    self.fields["assign_to_user"].initial = CustomUser.objects.get(
                        id=self.request.user.id
                    )

                self.fields["comment"].widget = forms.Textarea(
                    attrs={"placeholder": "Please provide your comments"}
                )
        ...
        def save(self, commit=True):
            ...
            if uploaded_files:
                file_list = handle_uploaded_file(
                    self.request, uploaded_files, format="file"
                )
                for f in file_list:
