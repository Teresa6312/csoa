...
from jsonForm.WorkflowExecutor import WorkflowExecutor  # Import the WorkflowExecutor class

logger = logging.getLogger('django')  # Get a logger instance

@receiver(pre_save, sender=Case)  # Decorator to connect the function to the pre_save signal of the Case model
def case_saved(sender, instance, **kwargs):
    ...
    if form is None:  # Check if the form is set
        raise ValueError("The 'form' cannot be empty.")  # Raise error if form is missing

    # Case was cancelled or only saved as draft
    if instance.status == constants.CASE_CANCELLED:  # If the case is cancelled
        if instance.task_instances.all().count() > 0:  # Check if there are any active task instances
            for task_instance in instance.task_instances.filter(is_active=1):  # Iterate through active task instances
                task_instance.is_active = False  # Deactivate the task instance
                task_instance.save()  # Save the changes
        if instance.workflow_instance is not None:  # Check if a workflow instance exists
    ...
    # Initial the workflow and task when user submit the form case
        ...
        if next_task is not None:  # If there is a next task
            instance.status = next_task.name  # Update the case status with the next task name
    ...
    elif instance.workflow_instance is not None:  # If a workflow instance exists (case is in progress)
        remain_task = instance.task_instances.filter(is_active=1)  # Get the remaining active task instances
        if remain_task.count() == 0:  # If there are no remaining active tasks
            priority_decision = wf_executor.get_priority_decision(instance)  # Get the priority decision point
            if priority_decision is not None and priority_decision.next_task is not None:  # If a decision point and next task exist
                next_task = wf_executor.execute(instance, priority_decision.next_task)  # Execute the workflow from the next task
                if next_task is not None:  # If there is a next task
