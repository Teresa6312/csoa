...
from .models import Company, Department, Team, CustomUser, AppMenu, Permission, CustomGroup
from .forms import CustomUserChangeForm, CustomUserCreationForm, AppMenuUpdateForm, PermissionInlineFormset, PermissionlineForm, CustomGroupForm, AppMenuCreateForm, AppMenuInlineCreateForm, AppMenuInlineUpdateForm
from base.admin import BaseAuditAdmin, default_readonly_fields
class DepartmentInline(admin.TabularInline):
    model = Department
    can_delete = False
    extra = 1
    fields = ['short_name', 'full_name','senior_manager', 'manager', 'is_active',]
...
class UserInline(admin.TabularInline):
    ...
    can_delete = False
    extra = 1
    fields = ['username', 'first_name', 'last_name','email', 'is_active' , 'department', 'position']
...
class DepartmentAdmin(BaseAuditAdmin):
    inlines = (TeamInline, UserInline, )
    list_display = ['company', 'full_name', 'short_name','is_active', 'senior_manager','manager']
        ...
        if self.parent_model:
            ...
            if parent_obj_id is not None:
                ...
                if g.group_type in CustomGroup.get_company_group_type():
                    return qs.filter(role__isnull=True, app__control_type__in=['company', 'department', 'team'])
                if g.group_type in CustomGroup.get_department_group_type():
                    return qs.filter(role__isnull=True, app__control_type__in=['department', 'team'])
...
class CustomUserAdmin(UserAdmin, BaseAuditAdmin):
    ...
    list_display = ['username', 'company','department', 'first_name', 'last_name','email', 'is_active', 'is_superuser']
    search_fields = ['username','first_name', 'last_name', 'email']
    readonly_fields = ('last_login', 'date_joined')
    list_filter = ['department', 'company', 'team']
    fieldsets = [
        (None,         {'fields': ('username', 'password')}),
        ('Personal info', {'fields': ('first_name', 'last_name', 'title', 'email', 'office_phone', 'mobile', 'company','department', 'team', 'position')}),
        ('Permissions', {'fields': ('is_active', 'is_staff', 'is_superuser', 'permissions')}),
        ('Important dates', {'fields': ('last_login', 'date_joined')+ default_readonly_fields}),
    ]
...
class AppFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        ...
        specific_instances = AppMenu.objects.filter(menu_level=0)  # Replace with your specific IDs
...
class CustomGroupAdmin(BaseAuditAdmin):
    ...
    inlines = (SelectPermissionline, PermissionInline,)
