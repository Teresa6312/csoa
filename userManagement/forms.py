        ...
        if self.cleaned_data["permissions"]:
            for perm in self.cleaned_data["permissions"]:
                ...
                if perm.role is None:
                    raise ValidationError(f"'{perm}' permission has no role")
                if (
                    g.group_type in CustomGroup.get_team_belongs_group_type()
                    and perm.team not in self.instance.team.all()
                ):
                    raise ValidationError(
                        f"'{perm}' permission is only applicable for users in '{perm.team}' team(s)"
                    )
                if (
                    g.group_type in CustomGroup.get_department_belongs_group_type()
                    and perm.department != self.instance.department
                ):
                    raise ValidationError(
                        f"'{perm}' permission is only applicable for users in '{perm.department}' department"
                    )
                if (
                    g.group_type in CustomGroup.get_company_belongs_group_type()
                    and perm.company != self.instance.company
                ):
                    raise ValidationError(
...
class AppMenuInlineUpdateForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(*args, **kwargs)
        if (
            self.parent_instance is not None
            and self.parent_instance.pk is not None
            and self.parent_instance.menu_level is not None
        ):
            self.fields["menu_level"].initial = self.parent_instance.menu_level + 1


class AppMenuInlineCreateForm(forms.ModelForm):
    is_form = forms.BooleanField(
        required=False, help_text="If yes, it will auto create 'form' related menus"
    )
    is_my_cases = forms.BooleanField(
        required=False, help_text="If yes, it will auto create 'My Cases' related menus"
    )
    is_search_cases = forms.BooleanField(
        required=False,
        help_text="If yes, it will auto create 'Search Cases' related menus",
    )
    has_my_case_list = forms.BooleanField(
        required=False,
        help_text="If yes, it will auto create 'Todo list','On Going list', and 'Completed List'",
    )
    has_cases_search_menus = forms.BooleanField(
        required=False, help_text="If yes, it will auto case search list"
    )
    ...
    def save(self, commit=True, *args, **kwargs):
        instance = super(AppMenuInlineCreateForm, self).save(
...
class AppMenuUpdateForm(forms.ModelForm):
    companys = forms.MultipleChoiceField(
        required=False,
        help_text="Only use when 'Control Type' select 'Company'",
        choices=[],
    )
    departments = forms.MultipleChoiceField(
        required=False,
        help_text="Only use when 'Control Type' select 'Department'",
        choices=[],
    )
    teams = forms.MultipleChoiceField(
        required=False,
        help_text="Only use when 'Control Type' select 'Team'",
        choices=[],
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["departments"].choices = get_select_choices_ids(
            "department_list_active"
        )
        self.fields["teams"].choices = get_select_choices_ids("team_list_active")

        self.fields["companys"].initial = [
            p["company"]
            for p in Permission.objects.filter(app_id=self.instance.id)
            .distinct()
            .values("company")
        ]
        ...
        if self.instance and self.instance.pk and self.instance.menu_level == 0:
            ...
            self.fields["parent_app_menu"].queryset = AppMenu.objects.filter(
                menu_level=-1
            )
    ...
    def save(self, *args, **kwargs):
        ...
        if instance_id is not None:
            ...
            linked_department = {
                "%s" % i["id"]: i["department_id"] for i in system_team
            }
            linked_company = {
                "%s" % i["id"]: i["company_id"] for i in system_department
            }

            if companys and self.cleaned_data["control_type"] == "company":

                existed = (
                    Permission.objects.filter(app_id=instance_id)
                    .distinct()
                    .values("company")
                )
                for e in existed:
                    if e["company"] not in companys:
                        Permission.objects.filter(
                            app_id=instance_id, company_id=e["company"]
                        ).delete()
                for c in companys:
                    if (
                        Permission.objects.filter(
                            app_id=instance_id, company_id=c
                        ).count()
                        == 0
                    ):
            ...
            if departments and self.cleaned_data["control_type"] == "department":
                ...
                for e in existed:
                    if e["department"] not in departments:
                        Permission.objects.filter(
                            app_id=instance_id, department_id=e["department"]
                        ).delete()
                for c in departments:
                    if (
                        Permission.objects.filter(
                            app_id=instance_id, department_id=c
                        ).count()
                        == 0
                    ):
                        Permission.objects.create(
                            app_id=instance_id,
                            department_id=c,
                            company_id=linked_company.get(str(c)),
                        )
            ...
            if teams and self.cleaned_data["control_type"] == "team":
                ...
                for e in existed:
                    if e["team"] not in teams:
                        Permission.objects.filter(
                            app_id=instance_id, team_id=e["team"]
                        ).delete()
                for c in teams:
                    if (
                        Permission.objects.filter(app_id=instance_id, team_id=c).count()
                        == 0
                    ):
                        ...
                        Permission.objects.create(
                            ...
                            company_id=linked_company.get(str(linked_department_id)),
                        )
...
class PermissionlineForm(forms.ModelForm):
    existing_permission = forms.ModelChoiceField(
        queryset=Permission.objects.filter(role=0),
        required=False,
        label="Select Existing Permission",
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.parent_instance:
            ...
            if self.parent_instance.id is not None:
                ...
                self.fields["existing_permission"].queryset = Permission.objects.filter(
                    filter
                )
...
class CustomGroupForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance.id:
            ...
            menu0 = AppMenu.objects.filter(
                is_active=True, menu_level=0, id__in=apps
            ).values("id")
            menu1 = AppMenu.objects.filter(
                is_active=True, menu_level=1, parent_app_menu__in=menu0
            ).values("id")
            menu2 = AppMenu.objects.filter(
                is_active=True, menu_level=2, parent_app_menu__in=menu1
            ).values("id")
            menu3 = AppMenu.objects.filter(
                is_active=True, menu_level=3, parent_app_menu__in=menu2
            ).values("id")
            menu4 = AppMenu.objects.filter(
                is_active=True, menu_level=4, parent_app_menu__in=menu3
            ).values("id")
            menu5 = AppMenu.objects.filter(
                is_active=True, menu_level=5, parent_app_menu__in=menu4
            ).values("id")
            menu_ids = (
                menu0.union(menu1).union(menu2).union(menu3).union(menu4).union(menu5)
            )
            self.fields["menus"].queryset = AppMenu.objects.filter(
                is_active=True, id__in=menu_ids
            ).order_by("menu_level", "parent_app_menu")
            if (
                self.instance.name == "User Admin"
                or self.instance.group_type is None
                or self.instance.group_type == 1
            ):
                self.fields["permissions"].queryset = (
                    DjangoPermission.objects.all()
                )  # 可选，设置权限的查询集
