...
class Company(BaseAuditModel):
    ...
    @global_class_cache_decorator(
        cache_key="company_list", timeout=settings.CACHE_TIMEOUT_L3
    )
    def get_list(cls):
    ...
    @global_class_cache_decorator(
        cache_key="company_list_active", timeout=settings.CACHE_TIMEOUT_L3
    )
...
class Department(BaseAuditModel):
    ...
    company = models.ForeignKey(
        Company, on_delete=models.PROTECT, related_name="department_company"
    )
    manager = models.ForeignKey(
        "CustomUser",
        on_delete=models.PROTECT,
    ...
    senior_manager = models.ForeignKey(
        "CustomUser",
        on_delete=models.PROTECT,
        related_name="department_senior_manager",
        blank=True,
        null=True,
    )
    is_active = models.BooleanField(default=True)
    ...
    @global_class_cache_decorator(
        cache_key="department_list", timeout=settings.CACHE_TIMEOUT_L3
    )
    def get_list(cls):
    ...
    @global_class_cache_decorator(
        cache_key="department_list_active", timeout=settings.CACHE_TIMEOUT_L3
    )
    def get_active_list(cls):
...
class Team(BaseAuditModel):
    ...
    department = models.ForeignKey(
        Department, on_delete=models.PROTECT, related_name="team_department"
    )
    manager = models.ForeignKey(
        "CustomUser",
        on_delete=models.PROTECT,
    ...
    first_support = models.ForeignKey(
        "CustomUser",
        on_delete=models.PROTECT,
        related_name="team_first_support",
        blank=True,
        null=True,
    )
    is_active = models.BooleanField(default=True)
    ...
    @global_class_cache_decorator(
        cache_key="team_list", timeout=settings.CACHE_TIMEOUT_L3
    )
    def get_list(cls):
    ...
    @global_class_cache_decorator(
        cache_key="team_list_active", timeout=settings.CACHE_TIMEOUT_L3
    )
    def get_active_list(cls):
...
class AppMenu(BaseAuditModel):
    ...
    key = models.CharField(
        max_length=31,
        validators=[get_validator("no_space_str_w_")],
        help_text="Unique identifier for the menu item (no spaces or special characters except '_').",
    )
    label = models.CharField(
        max_length=63, help_text="Display label for the menu item."
    )
    icon = models.CharField(
        max_length=31,
        blank=True,
        null=True,
        help_text="Icon class or name for the menu item.",
    )
    link = models.CharField(
        max_length=255,
        blank=True,
        null=True,
        help_text="URL or link associated with the menu item.",
    )
    control_type = models.CharField(
        max_length=31,
        choices=CONTROL_TYPE_CHOICE,
        blank=True,
        null=True,
        help_text="Type of control (e.g., 'App', 'Department', 'Team'). If empty, this is not a top-level menu/app.",
    )
    parent_app_menu = models.ForeignKey(
        "self",
        on_delete=models.CASCADE,
        blank=True,
        null=True,
        related_name="sub_menus",
        help_text="Parent menu item. Null for top-level menus.",
    )  # Changed 'AppMenu' to 'self'
    menu_level = models.PositiveSmallIntegerField(
        default=0,
        validators=[MaxValueValidator(5)],
        help_text="Menu level (0 for top-level, 1 for first sub-level, etc.). Max 5 levels.",
    )
    is_active = models.BooleanField(
        default=False, help_text="Whether the menu item is active and visible."
    )
    diango_view = models.CharField(
        max_length=127,
        blank=True,
        null=True,
    ...
    unit_control = models.BooleanField(
        default=False,
        help_text="Whether unit (department/team) control is enforced for this menu item.",
    )

    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["key", "label", "link", "menu_level", "parent_app_menu"],
                name="unique App menu",
            ),
            models.UniqueConstraint(
                fields=["key", "menu_level", "parent_app_menu"],
                name="unique App menu key",
            ),
        ]
    ...
    def save(self, *args, **kwargs):
        ...
        """
        if (self.control_type is not None and self.menu_level != 0) or (
            self.control_type is None and self.menu_level == 0
        ):
        ...
        if (
            self.menu_level == 0
            and AppMenu.objects.filter(
                label=self.label, control_type=None, menu_level=0
            ).exists()
        ):
            raise ValidationError(
                '"label" for top-level menus must be unique.', code="invalid_input"
            )
        super().save(*args, **kwargs)
    ...
    def build_menu_tree(cls, menu_items, user=None):
        ...
        """
        menu_items = menu_items.prefetch_related(
            "group_menus__permission_role"
        )  # Optimize query
        menu_dict = {}
        ...
        for item in menu_items:
            if user:
                role_unit = list(
                    item.group_menus.filter(
                        is_active=True, permission_role__user_permissions=user
                    )
                    .values(
                        "id",
                        "name",
                        ...
                        "permission_role__company__id",
                        "permission_role__department__short_name",
                        "permission_role__department__id",
                        "permission_role__team__short_name",
                        "permission_role__team__id",
                    )
                    .distinct()
                )
            else:
                role_unit = list(
                    item.group_menus.filter(is_active=True)
                    .values(
                        "id",
                        "name",
                        "group_type",
            ...
            for p in role_unit:
                p_app = (
                    p.get("permission_role__app__key") or ""
                ).lower()  # Simplified conditional
                if item.unit_control:
                    department = (
                        p.get("group_menus__permission_role__department__short_name")
                        or ""
                    ).lower()
                    team = (
                        p.get("group_menus__permission_role__team__short_name") or ""
                    ).lower()
                    if f"/{p_app}/" in link and (
                        f"/{department}/{team}/" in link
                        or f"/{department}/all/" in link
                    ):
                        new_role.append(p)
            ...
            if item.unit_control and not new_role:  # More Pythonic check
                ...
                menu_dict[item.id] = {
                    ...
                    "parent_app_menu__id": (
                        item.parent_app_menu_id if item.parent_app_menu else None
                    ),  # Accessing id directly.
                    "parent_app_menu__key": (
                        item.parent_app_menu.key if item.parent_app_menu else None
                    ),  # Accessing id directly.
        ...
        for item in menu_items:
            if (
                item.parent_app_menu
                and menu_dict.get(item.parent_app_menu_id)
                and menu_dict.get(item.id)
            ):  # Combined and more efficient condition
                menu_dict[item.parent_app_menu_id]["sub_menu"].append(
                    menu_dict[item.id]
                )
    ...
    @global_class_cache_decorator(
        cache_key="menu_tree_active", timeout=settings.CACHE_TIMEOUT_L3
    )
    ...
    def get_menu_tree_by_id_key(cls, menu_id, menu_key):
        ...
        return next(
            (
                m
                for m in menu_list
                if m.get("id") == menu_id and m.get("key") == menu_key
            ),
            None,
        )  # Corrected to m.get('id')

    @global_class_cache_decorator(
        cache_key="app_tree_active", timeout=settings.CACHE_TIMEOUT_L3
    )
    ...
    @global_class_cache_decorator(
        cache_key="app_instance", timeout=settings.CACHE_TIMEOUT_L3
    )
    def get_app_instance_by_key(cls, app_name):
        ...
        try:
            app = cls.objects.get(
                key=app_name, menu_level=0, is_active=True
            )  # Use .get() directly, more efficient.
            return app
        except cls.DoesNotExist:
            return None

    @global_class_cache_decorator(
        cache_key="app_form", timeout=settings.CACHE_TIMEOUT_L3
    )
    def get_app_form_by_id(cls, app_id):
        ...
        return (
            forms.first() if forms.exists() else None
        )  # More concise way to return None if no forms
    ...
    @global_class_cache_decorator(
        cache_key="menu_permission_roles_by_url", timeout=settings.CACHE_TIMEOUT_L3
    )
    def get_permission_roles_by_url(cls, url, department=None, team=None):
        ...
        if department is not None:
            queryset = queryset.filter(
                group_menus__permission_role__department=department
            )
    ...
    @global_class_cache_decorator(
        cache_key="menu_permission_users_by_url", timeout=settings.CACHE_TIMEOUT_L3
    )
    def get_permission_users_by_url(cls, url, department=None, team=None):
        ...
        queryset = cls.objects.filter(
            link=link, group_menus__is_active=True, is_active=True
        )  # Ensure the menu is active (_
        queryset.prefetch_related(
            "group_menus__permission_role"
        )  # Optimize queryset with prefetch_related
        ...
        """
            ...
            .annotate(
                ...
                first_name=F(
                    "group_menus__permission_role__user_permissions__first_name"
                ),
                last_name=F(
                    "group_menus__permission_role__user_permissions__last_name"
                ),
            )
            .distinct()
        )
...
class CustomGroup(
    Group, BaseAuditModel
):  # Assuming BaseAuditModel is defined elsewhere
    """
    ...
    group_type = models.PositiveSmallIntegerField(
        choices=constants.GROUP_TYPE_CHOICE,
        blank=True,
        null=True,
        help_text="""
    ...
    menus = models.ManyToManyField(
        AppMenu, related_name="group_menus", blank=True
    )  # Menus associated with this role
    ...
    @global_instance_cache_decorator(
        cache_key="group_active_user_list", timeout=settings.CACHE_TIMEOUT_L3
    )
    def get_active_users(self):
        """Gets a list of users associated with this group."""
        return self.user_set.filter(
            is_active=True
        )  # Use the built-in user_set attribute
...
class Permission(BaseAuditModel):  # Assuming BaseAuditModel is defined elsewhere
    ...
    role = models.ForeignKey(
        ...
        blank=True,
        null=True,
    )  # Role that this permission is granted to
    app = models.ForeignKey(
        AppMenu, on_delete=models.CASCADE, related_name="permission_app"
    )  # App that this permission applies to
    ...
    )  # Team scope for the permission (optional)
    department = models.ForeignKey(
        Department,
        on_delete=models.CASCADE,
        blank=True,
        null=True,
        related_name="permission_department",
    )  # Department scope for the permission (optional)
    company = models.ForeignKey(
        Company,
        on_delete=models.CASCADE,
        blank=True,
    ...
    )  # Company scope for the permission (optional)

    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["role", "app", "team", "department", "company"],
                name="unique permisson",
    ...
    def get_assign_to_role(cls, app, role, case):
        ...
        if role.name == constants.ROLE_CASE_OWNER:
            filter = Q(
                app=app,
                user_permissions=case.created_by,
                role__menus__key=f"createCase{case.form.code}",
            )
        elif (
            app.control_type != "app"
            and role.group_type in CustomGroup.get_team_group_type()
        ):
            filter &= Q(team=case.case_team)  # Filter by case team
        elif (
            app.control_type != "app"
            and role.group_type in CustomGroup.get_department_group_type()
        ):
            filter &= Q(
                team__isnull=True, department=case.case_department
            )  # Filter by case department
        elif (
            app.control_type != "app"
            and role.group_type in CustomGroup.get_company_group_type()
        ):
            filter &= Q(
                team__isnull=True,
                department__isnull=True,
                company=case.case_department.company,
            )  # Filter by case company
        else:
            filter &= Q(
                team__isnull=True, department__isnull=True, company__isnull=True
            )  # No data control
...
class CustomUser(AbstractUser, BaseAuditModel):
    ...
    office_phone = models.CharField(
        validators=[get_validator("phone_regex")],
        max_length=15,
        blank=True,
        null=True,
        help_text='Format:"000-000-0000"',
    )
    ...
    def __str__(self) -> str:
        ...
        """
        return (
            f"{self.title} {self.first_name} {self.last_name} ({self.username})"
            if self.title
            else f"{self.first_name} {self.last_name} ({self.username})"
        )

    @global_instance_cache_decorator(
        cache_key="user_menu_tree", timeout=settings.CACHE_TIMEOUT_L2
    )
    def get_user_menu_tree(self):
        ...
        """
        menu_items = (
            AppMenu.objects.filter(
                group_menus__permission_role__user_permissions=self, is_active=True
            )
            .distinct()
            .order_by("menu_level")
        )
        menu_dict = AppMenu.build_menu_tree(
            menu_items, self
        )  # Use AppMenu's build_menu_tree method
        return [
            v for k, v in menu_dict.items() if v is not None
        ]  # Filter out None values

    @global_instance_cache_decorator(
        cache_key="user_app_tree", timeout=settings.CACHE_TIMEOUT_L2
    )
    ...
    @global_class_cache_decorator(
        cache_key="user_list", timeout=settings.CACHE_TIMEOUT_L3
    )
    def get_list(cls):
        """
    ...
    @global_class_cache_decorator(
        cache_key="user_list_active", timeout=settings.CACHE_TIMEOUT_L3
    )
    def get_active_list(cls):
        """
    ...
    def get_user_info(cls, id):
        ...
        fields = set(cls.get_selected_fields_info([f.name for f in cls._meta.fields]))
        selected_fields = set(cls.selected_fields_info())
        fields = list(fields.union(selected_fields))
